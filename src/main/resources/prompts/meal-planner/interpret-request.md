Return a typed request-interpretation JSON object for the supported meal-planner workflow.
...
If the household request does not include pasted recipe text, set `oneOffRecipeContext` to `null`. Do not return `{}` for absent recipe context.
