# Product: Personal Meal Planning Agent
...
This is a private assistant for one household, not a public SaaS meal planner and not a calorie-tracking app. The product should make weekly lunch planning easier than doing it manually by combining stable household preferences, weekly constraints, recipe backlog ideas, practical health defaults, ingredient reuse, plan approval, shopping-list generation, and weekly feedback.

### Details

The app supports a shared household interface used by either Thomas or Cassandra. There is **one UI role** in v0: **Household Member**. Thomas and Cassandra are still represented separately inside the profile for food preferences, dislikes, and feedback, but the UI does not have different permissions, dashboards, or separate role-specific flows.

The primary planning unit is **weekday lunch**. The default plan is **5 weekday lunches**, with optional dinners added only when requested. Breakfast remains profile-based and mostly unchanged unless ingredients are needed for the shopping list.

The app should also include an always-available **Recipe Explorer**. Household members can discover or capture recipes at any time and add them to a **Recipe Backlog**. During weekly planning, especially before shopping, the agent can suggest backlog recipes that fit the week.
...
| No separate Thomas/Cassandra UX permissions in v0                                         | Locked                             |
...
| Shopping list generated after plan approval                                               | Locked                             |
| Recipe Explorer is always available                                                       | Locked                             |
| Recipe Backlog can feed into weekly planning                                              | Locked                             |
| Health is supportive, approximate, and non-shaming                                        | Locked                             |
| Exact calorie tracking, grocery ordering, public SaaS, and multi-household support        | Out of scope                       |
...
| Gap                                                                      | Impact                                                                                                                      |
| ------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------- |
| Existing spec defines product behavior but not concrete UX screens       | Downstream work cannot verify what appears on each page.                                                                    |
| Role model was ambiguous                                                 | Now locked: one shared **Household Member** role.                                                                           |
| Cassandra involvement was described, but separate UI role is not desired | Cassandra preferences and feedback exist, but no Cassandra-only screen in v0.                                               |
| Entry points were not fully defined                                      | This spec adds dashboard, nav, plan, shopping, feedback, and recipe explorer entry points.                                  |
| Recipe discovery was previously under-specified                          | Now added as an always-available Recipe Explorer with backlog.                                                              |
| State transitions were not explicit                                      | This spec defines what the household sees before, during, and after generation, revision, approval, shopping, and feedback. |
| Empty states were missing for aggregate UIs                              | This spec defines empty states for plans, lists, meals, recipe backlog, shopping sections, and feedback.                    |
| Mobile behavior was not locked                                           | This spec gives provisional mobile behavior: stacked cards, sticky bottom actions, compact nav.                             |
| Access/session model is not locked                                       | UX assumes a shared household session; enforcement/authentication belongs to tech.                                          |
| Real-time transport is not locked                                        | UX assumes active-screen updates plus stale-state banners for other tabs; SSE/websocket/polling belongs to tech.            |
...
There is exactly **one interactive UI role** in v0:
...
| ------------------- | --------------------------: | ------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Household Member    |                         Yes | All v0 screens: dashboard, profile, planner, plan review, recipe explorer, recipe backlog, shopping list, feedback | Start planning, edit inputs, revise meals, approve plan, generate shopping list, check shopping items, give feedback, add recipes to backlog | This role represents either Thomas or Cassandra. No per-person UI permission split.       |
...
| Cassandra           |         No separate UI role | Appears as a person inside profile/preferences/feedback                                                            | Same as Household Member                                                                                                                     | Cassandra has no separate login-specific journey in v0 unless access model changes later. |
...
| --------- | ------------------------------- | --------------------------: | -------------------------------------------- |
| Dashboard | `/`                             |                         Yes | Shows current week state and next action.    |
...
| ---------------------------- | --------------------------------------------------------------- | ---------------------------------------------------------- | ----------------------------------- | ------------------------------------------------------- |
| Start weekly planning        | Dashboard empty/current-week card                               | `/plan/new?weekStart=YYYY-MM-DD`                           | `Plan this week` / `Plan next week` | Main v0 entry point.                                    |
| Review active draft          | Dashboard active draft card                                     | `/plans/{planId}`                                          | `Review draft plan`                 | Appears after generation.                               |
| Continue approved plan       | Dashboard approved plan card                                    | `/plans/{planId}`                                          | `Open weekly plan`                  | Shows approved meals and shopping state.                |
| Generate shopping list       | Approved plan page                                              | `/plans/{planId}/shopping-list`                            | `Generate shopping list`            | Only after approval.                                    |
...
| Add backlog recipe into plan | Weekly planning input or draft plan                             | `/recipes/backlog?mode=selectForPlan&weekStart=YYYY-MM-DD` | `Add from backlog`                  | Lets household choose backlog candidates for this week. |
...
| ---------------------------------------------------------- | ------------------- | ------------------------------------------------------------------------------- | ------------- | ----------------- | -------------------------------------------------------------------- |
| `/plan/new?weekStart=YYYY-MM-DD`                           | `weekStart`         | Anchors planning to a specific week.                                            |               |                   |                                                                      |
...
| `/plans/{planId}/shopping-list`                            | `planId`            | Opens shopping list for an approved plan.                                       |               |                   |                                                                      |
...
| Shared household session/cookie                            | Provisional         | UX assumes a shared household context. Login/session enforcement is tech scope. |               |                   |                                                                      |
| Per-person role token                                      | Not needed in v0 UX | There are no separate Thomas/Cassandra role views.                              |               |                   |                                                                      |
...
7. App returns to dashboard with `Plan this week`.
...
Completion returns to `/` with the next action card: `Plan this week`.
...
Generate a concrete lunch-first weekly plan and approve it before creating a shopping list.
...
9. Household reviews:
    ...
    * prep notes
    * fallback meal
10. Household clicks `Approve plan`.
11. Approved state appears.
...
| Generation fails           | Error card: “Couldn’t create a plan. Keep your inputs and try again.” Buttons: `Try again`, `Simplify this week`, `Edit inputs`. |
| Plan feels wrong           | Buttons on plan: `Revise plan`, `Swap meal`, `Make easier`, `Start from scratch`.                                                |
| User exits before approval | Dashboard shows draft plan card with `Continue review`.                                                                          |
...
Let the household discover recipes before shopping day, then let the agent suggest backlog recipes that fit the weekly plan.
...
7. On weekly planning day, planning form shows:
    ...
    * “Backlog recipes that might fit this week”
...
| Saved recipe no longer fits current week  | Show it as disabled or lower priority with reason: “Too much effort for this week.” |
...
## Journey 5 — Plan approval and shopping list

### Goal

Approve the weekly plan first, then generate an editable, grouped shopping list.

### Big Picture

The shopping list should not be generated from an unstable draft. The plan must reach an approved state first.

### Details

1. Household member opens draft plan.
2. Reviews meal cards, reuse notes, fallback, and optional dinners.
3. Clicks `Approve plan`.
4. Approved state appears:
...
9. Checked items remain visible but muted/collapsed depending on view setting.

### Empty/error/escape branches

| Branch                                   | UX behavior                                                                              |
| ---------------------------------------- | ---------------------------------------------------------------------------------------- |
| Plan has no meals                        | Approval disabled with message: “Add at least one planned meal before approval.”         |
| Shopping list generation fails           | Keep approved plan. Show `Try again` and `Edit plan`.                                    |
...
| Branch                              | UX behavior                                                                                   |
| ----------------------------------- | --------------------------------------------------------------------------------------------- |
| No approved plan exists             | Feedback page shows empty state: “No completed plan to review yet.” Button: `Plan this week`. |
| Household skips meal-level feedback | Allow save if at least one quick answer is selected, or allow `Skip this week`.               |
...
| No active plan         | `Plan this week`         | Opens `/plan/new?weekStart=YYYY-MM-DD`.          |
| Draft exists           | `Review draft plan`      | Opens `/plans/{planId}`.                         |
| Approved plan, no list | `Generate shopping list` | Opens approved plan or shopping-list generation. |
...
| State                | UI                                                             |
| -------------------- | -------------------------------------------------------------- |
| No plan              | Friendly empty card: “No plan for this week yet.”              |
...
|                                                             |
| [ No plan yet ]                                             |
| Plan 5 practical high-protein lunches for this week.         |
|                                                             |
| [Plan this week]      [Explore recipes]                     |
+-------------------------------------------------------------+
| Recipe backlog                                              |
...
| `Plan this week`    | Appears after basics complete; opens weekly planning.   |
...
| [Add pantry item]                                           |
|                                                             |
| [Save basics] [Save profile] [Plan this week]               |
+-------------------------------------------------------------+
...
| Component                   | Visible? | Notes                                                                                                 |
| --------------------------- | -------: | ----------------------------------------------------------------------------------------------------- |
| Week selector               |      Yes | Defaults to current or next week.                                                                     |
...
| Cravings/dislikes this week |      Yes | Temporary weekly modifiers.                                                                           |
...
| No candidate recipes fit        | “Nothing in backlog looks ideal for this week.” Buttons: `Ignore backlog`, `Explore recipes`. |
...
|                                                             |
| Cravings, dislikes, avoid this week                         |
...
|                                                             |
| Extra context                                               |
| [ Anything else the agent should know this week _______ ]    |
|                                                             |
...
**Path:** `/plans/{planId}`
**Role:** Household Member
**Purpose:** Review, revise, and approve the generated weekly plan.

### Components visible

| Component                |      Visible? | Notes                                              |
| ------------------------ | ------------: | -------------------------------------------------- |
| Plan status banner       |           Yes | Draft, approved, shopping generated, feedback due. |
...
| Approval controls        |    Draft only | `Approve plan`, `Revise plan`.                     |
| Shopping CTA             | Approved only | `Generate shopping list` / `Open shopping list`.   |
...
| `Approve plan`           | Changes status to approved.                                                                                                                 |
| `Generate shopping list` | Starts shopping list generation after approval.                                                                                             |
...
| Optional dinners absent | “No dinners planned. Lunch is the focus this week.”                                  |
...
|                                                             |
| [Regenerate whole plan]                   [Approve plan]    |
+-------------------------------------------------------------+
| Monday lunch                                                |
| Chicken rice bowl                                           |
...
+-------------------------------------------------------------+
| Optional dinners                                            |
| No dinners planned. Lunch is the focus this week.            |
| [Add dinner]                                                |
+-------------------------------------------------------------+
| Assumptions                                                 |
...
Approved state:
+-------------------------------------------------------------+
| Week of Mon YYYY-MM-DD                      Status: Approved |
...
**Role:** Household Member
**Purpose:** Provide an editable, checkable, grouped shopping list after plan approval.
...
| State                       | UI                                                                                         |
| --------------------------- | ------------------------------------------------------------------------------------------ |
| No shopping list yet        | “Shopping list is generated after plan approval.” Button: `Open plan`.                     |
...
| Source: Approved plan                         [Back to plan]|
+-------------------------------------------------------------+
| Produce                                                     |
...
| State             | UI                                                           |
| ----------------- | ------------------------------------------------------------ |
| No approved plan  | “No completed plan to review yet.” Button: `Plan this week`. |
...
| Add manual recipe    | Optional/provisional | Useful if household has a known recipe.                    |
...
| `Use this week`       | In planning mode, marks recipe as candidate for weekly plan.            |
...
Planning context:
+-------------------------------------------------------------+
| This could fit the current week.                             |
...
| Action buttons     |      Yes | Save, use this week, mark avoid, back.      |
...
| `Use this week`   | Adds as candidate in current planning context. |
...
|                                                             |
| [Save to backlog] [Use this week] [Mark avoid]              |
+-------------------------------------------------------------+
```
...
Because v0 has one shared UI role, every transition is perceived by the **Household Member**. Thomas and Cassandra do not receive different role-specific views.

Real-time transport is not locked. UX behavior should not depend on a specific transport. Provisional v0 behavior:
...
| ----------------------------------- | ------------------------------- | ---------------------------------- | ---------------------------------------------------- | ---------------------------------------- | ---------------------------------------------------------------- |
| Profile incomplete → basics saved   | `Save basics`                   | Dashboard blocks planning or warns | Button loading                                       | Dashboard shows `Plan this week`         | Show updated profile state on refresh/focus.                     |
...
| Draft → approved                    | `Approve plan`                  | Draft controls visible             | Button loading                                       | Status: Approved; shopping CTA visible   | Other sessions show approved state after refresh/focus.          |
| Approved → shopping list generating | `Generate shopping list`        | Approved plan                      | Blocking loading                                     | Shopping list page                       | Other sessions show shopping list available after refresh/focus. |
...
| Approved plan with no shopping list  | `Generate shopping list`.                                                                     |
...
| Feedback unavailable because no plan | `Plan this week`.                                                                             |
| Plan generation error                | `Try again`, `Simplify this week`, `Edit inputs`.                                             |
...
| `/plans/{planId}`                                          | Household Member            | Household Member                                  | Revise, approve, regenerate, open shopping. |
...
| Approve plan             | `planId`                                                                   | Mark this weekly plan as accepted by the household.               |
| Generate shopping list   | `planId`                                                                   | Create grouped list from approved plan.                           |
...
| Use recipe this week     | `recipeId`; `weekStart` or `planId`; optional `mealSlotId`                 | Make recipe a candidate for current weekly plan or selected slot. |
...
| ---------------------------- | -------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------ |
| Dashboard active plan        | “No plan for this week yet.” Button: `Plan this week`.                                       | If draft incomplete, show partial draft and `Continue planning`. | Not applicable.                                                          |
...
| Optional dinners             | “No dinners planned. Lunch is the focus this week.” Button: `Add dinner`.                    | Show planned dinners only; do not imply error.                   | Not applicable.                                                          |
...
| Fallback meals               | Fallback is required; if missing, show error: “Add fallback before approval.”                | If only one fallback exists, show one.                           | If multiple equal fallback candidates, show top 2–3.                     |
...
| Backlog candidates for week  | “No backlog recipes look ideal for this week.” Buttons: `Ignore backlog`, `Explore recipes`. | Show fewer candidates and explain why.                           | If tied, prefer easier/lunch-suitable recipes.                           |
...
| Plan history, if shown later | “No previous plans yet.” Button: `Plan this week`.                                           | Show available history.                                          | Sort ties by most recent.                                                |

---

## UX-specific product rules

### Approval and shopping

* Draft plans can be revised freely.
* The shopping list is generated only after the plan is approved.
...
* Weekly planning can suggest backlog recipes that fit the current week.
...
* No exact calorie compliance UI in v0.
...
* Household opens dashboard with no profile basics → completes basics → returns to dashboard → sees `Plan this week`.
...
* Household approves draft plan → shopping-list CTA appears.
* Household tries to open shopping list before approval → sees “generated after plan approval” empty state.
* Household generates shopping list from approved plan → sees grouped sections and pantry assumptions.
* Household checks shopping items → items become checked/muted and remain in list.
* Household edits approved plan after shopping list generation → shopping list shows outdated banner.
...
* Household opens feedback with no approved plan → sees empty state and `Plan this week`.
...
* **US-16**: As a household member, I can choose weekly priorities like easy, cheap, high-protein, lighter, comforting, varied, use existing ingredients, or meal-prep friendly so that the agent understands this week’s bias.
...
* **US-20**: As a household member, I can explore recipes at any time so that I can build a backlog before shopping day.
...
* **US-24**: As a household member, I can select backlog recipes during weekly planning so that saved ideas can be integrated into the current week.
...
### Approval and shopping stories

* **US-38**: As a household member, I can approve a draft plan so that the app knows it is stable enough for shopping-list generation.
* **US-39**: As a household member, I cannot generate a final shopping list from an unapproved draft so that shopping does not reflect unstable meals.
* **US-40**: As a household member, I can generate a grouped shopping list after approval so that shopping is practical in the store.
...
## Open questions, uncertainties, and provisional decisions

These are the remaining UX decisions that should be explicitly confirmed. The spec above assigns provisional defaults so downstream work is not blocked.

| Question                                                                 | Provisional decision in this spec                                                                              | Why it matters                                                                        |
| ------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| Is there any login at all, or just a shared private session/link?        | Shared household context; no per-person UX roles.                                                              | Affects entry copy, session expiration, and whether links need visible access states. |
| Should first-time profile setup be forced or skippable?                  | Required basics only; optional fields skippable.                                                               | Affects onboarding friction.                                                          |
| Should weekly planning be wizard, single form, or chat?                  | Single compact form with optional free-text context.                                                           | Affects screen count and Playwright flows.                                            |
| Should dashboard be the primary app home?                                | Yes.                                                                                                           | Needed to lock entry points.                                                          |
| Should the app plan the current week or next week by default?            | Use dashboard copy `Plan this week` or `Plan next week` based on date/context; URL always carries `weekStart`. | Important around weekends and shopping day.                                           |
| What exact day is “shopping day”?                                        | Not locked. Planning flow allows any `weekStart`; no shopping-day-specific trigger yet.                        | Needed if the app should proactively surface backlog suggestions on a certain day.    |
| Should Recipe Explorer be swipe-based or button-based cards?             | Button-based cards required; swipe gesture optional.                                                           | Buttons are clearer for accessibility and testing.                                    |
| Should manual recipe entry exist in v0?                                  | Provisional yes as an escape hatch.                                                                            | Useful when explorer has no suggestions, but may expand scope.                        |
| How much nutrition appears on meal cards?                                | Rough protein and portion labels visible; rough calories optional when available.                              | The original spec supports approximate nutrition but avoids strict tracking.          |
| Should calories be visible by default?                                   | Not fully locked. This spec uses supportive labels and optional rough calorie/portion notes.                   | Calories can make the product feel more like a diet app.                              |
| How are two open devices synchronized?                                   | Active-screen update plus stale banner on focus/refresh; no transport locked.                                  | Real-time mechanism belongs to tech, but UX must show stale-state handling.           |
| Can a household member approve a plan without Cassandra-specific review? | Yes, because there is one shared role.                                                                         | Follows the locked one-role model.                                                    |
| Should final feedback require any answer?                                | Provisional: allow partial save or skip.                                                                       | Keeps feedback low-friction.                                                          |
| Should checked shopping items collapse automatically?                    | Provisional: checked items become muted; collapse can be a view option later.                                  | Affects shopping usability.                                                           |
| Should one experimental recipe per week be hard or soft?                 | Soft default; household can request more.                                                                      | Balances novelty with practical adherence.                                            |

The highest-impact unresolved UX decision is the **access model**: shared private session, shared login, or private link. It does not change the one-role interface, but it changes first-run, expired-session, and shared-link screens.
